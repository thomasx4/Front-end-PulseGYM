...
export class EditarSocioComponent implements OnInit {
  ...
  procesarEquipamiento(): void {
    ...
    this.equipamiento = equiposArray.map(nombre => ({ nombre }));
  ...
  volver(): void {
    this.router.navigate(['/trainer/rutinas/socio', this.idSocio, 'rutina', this.rutinaId]);
  }
  ...
  guardarCambios(): void {
    ...
    modificados.forEach((detalle, index) => {
      const payload = {
        ...
        motivo: `Ajuste del entrenador - ${new Date().toLocaleDateString()}`
      };

      this.entrenadorService.ajustarDetalle(this.rutinaId, payload).subscribe({
        next: (response: any) => {
          completados++;
